...
test('AchillesCLI bootstraps automatically and explains steps', { concurrency: false, timeout: 20_000 }, async () => {
    ...
    const planSteps = [{ skill: 'generate-summary', prompt: 'preview the specifications' }];
...
test('AchillesCLI focuses on specs before code generation', { concurrency: false, timeout: 25_000 }, async () => {
    ...
    const planSteps = [
        ...
        { skill: 'generate-summary', prompt: 'Preview the specification set.' },
    ];
    const handlers = {
        'sync-specs-plan': '[]',
    ...
    assert.ok(!fs.existsSync(path.join(workspace, '.specs', 'html_docs', 'index.html')), 'Full HTML docs should not be generated by generate-summary.');
});
    ...
    const llm = new LLMAgent({
        invokerStrategy: async ({ context, prompt }) => {
            if (context?.intent === PLAN_INTENT) {
                ...
                return JSON.stringify([{ skill: 'generate-summary', prompt: 'Preview specifications.' }]);
            }
            return '[]';
        },
    });
...
test('AchillesCLI routes spec summary requests through the planner', { concurrency: false, timeout: 10_000 }, async () => {
    ...
    const planSteps = [
        { skill: 'generate-summary', prompt: 'Summarise current specifications.' },
    ];
    const cli = new AchillesCLI({
        llmAgent: createPlannerLLM(planSteps),
        workspaceRoot: workspace,
