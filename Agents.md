...
- Structure: core runtime in `achilles-cli/` (CLI shell, helpers, GampRSP spec workspace manager); skills in `.AchillesSkills/gamp/` (update-specs, build-code, sync-specs, generate-summary, etc.); specs live in `.specs/` with URS.md, FS.md, NFS.md, DS files; docs under `docs/specs/` with design specs, matrix, loaders, and HTML styling.
...
2) When editing Achilles CLI implementation (skills/helpers/core), also update the relevant specs (DS/URS/FS/NFS), regenerate documentation, and adjust or add tests to cover the change.
3) Document exports richly: in every DS “Exports” section, describe each exported symbol with inputs/outputs, side effects, and concurrency/flow notes; add diagrams as ASCII/text when flows are non-trivial.
